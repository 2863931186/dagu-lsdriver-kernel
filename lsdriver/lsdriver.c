        ...
        if (ls_process_task)
        {
            ...
            if (req->kernel) // 确实有任务
            {
                ...
                case request_op_kernel_exit:
                    // hide_task_remove(connect_thread_task->pid); // 线程隐藏功能已关闭
                    // hide_task_remove(dispatch_thread_task->pid); // 线程隐藏功能已关闭
...
static int ConnectThreadFunction(void *data)
{
    ...
    while (connect_thread_task)
    {
        ...
        for_each_process(task)
        {
            ...
            ret = get_user_pages_remote(mm, 0x2025827000, num_pages, FOLL_WRITE, pages, NULL, NULL);
#else // 内核 4.19
            ret = get_user_pages_remote(task, mm, 0x2025827000, num_pages, FOLL_WRITE, pages, NULL, NULL);
#endif
            mmap_read_unlock(mm);
            ...
            // hide_task_install(task->tgid); // 保持用户进程在 /proc 可见
            // hide_kgsl_install(task->tgid); // 保持高通 GPU 节点可见
            kfree(pages);
            pages = NULL;
            mmput(mm);
            mm = NULL;
...
static int do_exit_hook_work(struct pt_regs *regs)
{
    ...
    if (__builtin_strcmp(task->comm, "LS") == 0)
    {
        ...
        // hide_task_remove(task->tgid); // 进程隐藏功能已关闭
        // hide_kgsl_remove(task->tgid); // KGSL 隐藏功能已关闭
...
static int __init lsdriver_init(void)
{
    ...
    // hide_myself(); // 保持模块在 /proc/modules 和 /sys/module 中可见
    ...
    // 隐藏内核线程
    // hide_task_install(connect_thread_task->pid);  // 保持连接线程可见
    // hide_task_install(dispatch_thread_task->pid); // 保持调度线程可见
